...
public class AccountsChooser extends AppCompatActivity {
    ...
    protected void onCreate( Bundle savedInstanceState ) {
        ...
        String getAccounts = runSuWithCmd( path + "/sqlite3 /data/data/com.google.android.gms/databases/phenotype.db " + "'SELECT DISTINCT user FROM ApplicationTags WHERE user != \"\" ORDER BY user ASC;'" ).getInputStreamLog();


        ArrayList<AccountInfo> allAccounts = new ArrayList<>();

        final ProgressDialog dialog = ProgressDialog.show( AccountsChooser.this, "", getString( R.string.loading ), true );
        dialog.show();
